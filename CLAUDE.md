...
just stress     # load harness (cmd/stress), see docs/performance.md
...
A self-hosted OpenAI-compatible LLM proxy: one static Go binary, per-user API keys, curated model catalog, usage accounting. Exactly two upstream wire formats (OpenAI-compatible and Anthropic native); the non-goals table in docs/architecture.md caps it there deliberately. Read docs/architecture.md before structural changes; it states every design rule with its rationale.
...
- `internal/config`: all configuration, environment-driven with `LLMPROXY_` prefix, defaults in `FromEnv()`. Document new variables in docs/configuration.md and expose them in `docker-compose.coolify.yml` as `${VAR:-default}`.
...
`docker-compose.yml` is the development setup (SQLite, loopback-only port). `docker-compose.coolify.yml` is the PaaS deployment (Postgres, no host port mapping, every `LLMPROXY_*` variable exposed as `${VAR:-default}` so Coolify lists them). Keep the two and docs/docker.md in sync when configuration changes.
