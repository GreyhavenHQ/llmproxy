...
just stress     # load harness, see docs/performance.md
```
...
- If you changed configuration, document the variable in
  `docs/configuration.md` and expose it in `docker-compose.coolify.yml` as
  `${VAR:-default}`.
- New behaviour comes with a test.

## Design rules that are not negotiable

Read [docs/architecture.md](docs/architecture.md) before any structural change.
...
The non-goals table in `docs/architecture.md` is deliberate. If a feature is
